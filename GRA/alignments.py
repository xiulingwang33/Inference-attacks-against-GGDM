	...
	if topk is None:
		...
		for node_index in range(n_nodes):
			...
			if sp.issparse(alignment_matrix):
				...
				print('444', sorted_indices[node_index])
				print('555', node_sorted_indices)
...
def score_alignment_matrix_diffusion(alignment_matrix, topk=None, topk_score_weighted=False, true_alignments=None):
	n_nodes = alignment_matrix.shape[0]
	correct_nodes = []
	...
	if topk is None:
		...
		if not sp.issparse(alignment_matrix):
			sorted_indices = np.argsort(alignment_matrix)
			print(np.sort(alignment_matrix))
	...
	return alignment_score, set(correct_nodes),alignment_matrix